        ...
        <View style={{ width: '85%' }}>
          <View style={styles.search}>
            <View style={{ flexDirection: 'row' }}>
              <SvgXml xml={searchIcon} style={{ marginRight: 10 }} />
              <TextInput value={search} onChangeText={setSearch} placeholder={'Suchen'} style={{ width: '67%' }} />
            </View>
            <TouchableOpacity
              activeOpacity={0.5}
              style={{
          ...
          search === "" && mappedData.map(({ letter, contacts }) => (
            <View key={letter}>
              ...
              {contacts.map(({ name, email }) => {
                ...
                return (
                  <TouchableOpacity activeOpacity={0.5} onPress={() => navigation.navigate("contact-details", {startColor, endColor, name, email})} key={name} style={styles.card}>
                    <LinearGradient
